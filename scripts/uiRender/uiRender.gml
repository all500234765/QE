...
#region
            ...
            matrix_add_translate(16 - (ext0[1] - 2) * 32, 16 - (ext0[0] - 2) * 32, 0);
            uiClipBegin(__uiX + nullVertex[0] + 16 * type[0], v[1] + 16, width, height - 16 * (1 + type[0] + f1));  // Clip begin
                
                uiRender(0, 0, data[5]);
                
            uiClipEnd();                                                                                            // Clip end
            matrix_add_translate((ext0[1] - 2) * 32 - 16, (ext0[0] - 2) * 32 - 16, 0);
            ...
            if( type[1] ) {                                                                                 // Vertical
                matrix_set(matrix_world, matrix_build_identity());
                matrix_add_translate(16, v[1], 0);
                var v = matrix_nullvertex();
                
                if( diff[1] > 0 ) {
                    var maxCount = diff[1] div 32;
                    var maxDraw = height div 32;
                    ...
                    if( matrix_rect(w - 16, 4, w, height) && !uiFlag(flags, uiFlags.Inactive) && __uiClickZ == nullVertex[2] ) {       // Move position
                        var sbID = (UICT << 10) + (i << 3);
                        if( mouse_check_button_pressed(mb_left) ) {
                            __uiSBID = sbID;
                        }
            ...
            if( type[0] ) {                                                                                 // Horizontal
                ...
                if( diff[0] > 0 ) {
                    ...
                    // Mouse is down and in rectangle
            ...
            matrix_set(matrix_world, matrix_build_identity());
            matrix_add_translate(16, v[1] + height, 0);
            //matrix_add_translate(0, height, 0);
#endregion
            break;
...
#region
            ...
            // Background
            draw_set_color(inactive * uiFlag(flags, uiFlags.Inactive) + make_color_rgb(gpBack[0] + In * data[2] + clickIn * q, gpBack[1] + In * data[2] + clickIn * q, gpBack[2] + In * data[2] + clickIn * q));
